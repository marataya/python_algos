...
class BinaryNode:
    ...
    def adjust_count\
                    (self,value,delta):
        """Adjust num_left count after value has been added."""
        if value < self.value:
            self.num_left += delta
            if self.left:
                (self.left.adjust_count
                 (value, delta))
        elif value > self.value:
            if self.right:
                (self.right.adjust_count
                 (value, delta))
    ...
    def __repr__(self):
        ...
        rightS = ''
        if self.left:
            leftS = str(self.left) + '@' + str(self.num_left)
        if self.right:
...
if __name__ == '__main__':
    b = CountingBinaryTree(1, 4,2, 3, 9, 8, 7, 6, 5)
    print(b)
