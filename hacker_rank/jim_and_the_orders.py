...
def jimOrders(orders):
    completion_times = [(order[0] + order[1], i + 1) for i, order in enumerate(orders)]
    completion_times.sort()
    result = [customer[1] for customer in completion_times]
    return result

if __name__ == '__main__':
    # Sample Input
    orders = [[8,2],[4,2],[5,6],[3,1],[4,3]]
    # Output the ordered customer numbers
